...
/**
 * OpenSpec storefront-main-navigation: header drives IA (drawer on small viewports).
 */
...
for (const layout of ["mobile", "desktop"] as const) {
  test.describe(`Storefront main navigation (${layout})`, () => {
    test.use(
      layout === "mobile"
    ...
    test("every primary header nav item reaches the correct page (guest)", async ({ page }) => {
      for (const dest of primaryNavGuest) {
        ...
        await expect(page.getByTestId(dest.landmark)).toBeVisible({ timeout: 15_000 });
      }
    });

    test("authenticated customer reaches Account via menu", async ({ page }) => {
      ...
      await storefrontNavLink(page, layout, "account").click();
